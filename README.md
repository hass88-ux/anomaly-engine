...
**Live:** [anomaly-engine-nzlk.onrender.com](https://anomaly-engine-nzlk.onrender.com)
Hosted on Render's free tier, so the first request after a period of inactivity takes
around 50 seconds while the container starts. Sign in with the demo account for a
populated alert queue, or upload your own CSV.

## Status
Complete and deployed. Engine tagged `v1-engine`. REST API with MySQL persistence, JWT
authentication and Hibernate-enforced tenant isolation. React interface covering rule
tuning, generated runs, CSV upload with column mapping and live progress, alert review,
and history. Running on Render against a managed Aiven MySQL instance.
...
1. `POST /api/uploads` stores the file, reads only the header and five rows, returns an
   upload ID with detected columns and a preview. A 25MB file returns in under a second
   because 99.99% of it is never touched.
2. The user confirms or corrects the mapping.
3. `POST /api/uploads/{id}/analyze` returns **202 Accepted** with a job ID and queues
   the work. 202 rather than 200 because the work is not done — it has been accepted.
4. `GET /api/jobs/{id}` reports status and progress.
...
- **Deployed on ephemeral storage.** The container has no persistent disk, so an uploaded
  file is lost if it restarts mid-analysis. The database is external, so completed results
  and review decisions survive; only in-flight uploads do not.
- **The free tier sleeps.** Both the web container and the free database plan power down
  when idle, so the first request after a quiet period takes around 50 seconds. That is a
  hosting choice rather than an application characteristic — the same artifact starts in
  13 seconds on a warm machine.
- **Uploaded files are stored on local disk** between upload and analysis. This works on a
  ...
  does not hold the file. S3 is the correct fix for both this and the point above.
...
## Roadmap
- S3-backed upload storage, removing the ephemeral-disk limitation
...
The application and the frontend ship as a single artifact. `mvn package` runs the Vite
build through the frontend-maven-plugin and copies `frontend/dist` into the JAR's static
resources, so Spring serves both the API and the interface from one origin. This removes
CORS from the deployed system entirely — the configuration is retained and externalised
only so the two can be split later if needed.

A multi-stage Dockerfile builds with Maven and a full JDK, then copies only the JAR into
a JRE-only runtime image. The final image is roughly a quarter the size of a single-stage
build, which matters on a 512MB container.

`-XX:MaxRAMPercentage=75` tells the JVM to size its heap against the container's limit
rather than the host's. Without it a JVM in a small container claims heap based on the
underlying machine and is killed by the OOM killer — the most common way a Spring Boot
app dies on constrained hosting.

React Router owns the client-side routes, so a browser requesting `/alerts` directly asks
the server for a path Spring has no mapping for. `SpaController` forwards those to
`index.html`. The routes are listed explicitly rather than using a catch-all, because a
wildcard would swallow genuine 404s and return HTML for any typo'd URL.

**Every secret is an environment variable with no default.** The application will not
start without `DB_PASSWORD` and `JWT_SECRET`, which is deliberate: a default would let a
misconfigured deployment run with a known signing key. This was verified the hard way —
the first attempt to run the packaged JAR failed on a missing `jwt.secret` placeholder,
which is exactly the failure that should happen.
...
## Data
City data derived from [GeoNames](https://www.geonames.org/), licensed under
[CC BY 4.0](https://creativecommons.org/licenses/by/4.0/). Filtered to populated places in
